...
class ProcessingPipeline(BasePipeline):
    ...
    def __init__(self, pipeline_name: str, processor_func: Callable[[List[Dict[str, str]], Path, Path], Dict[str, Any]]):
    ...
    def filter_sources(self, filter_func: Callable[[List[Dict[str, str]]], List[Dict[str, str]]]) -> List[Dict[str, str]]:
    ...
    def process_sources(self, sources: List[Dict[str, str]]) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(stats, dict):
                ...
                self.result.failed_items = stats.get("total_sources", len(sources)) - stats.get("processed_sources", 0)
    ...
    def execute(self) -> PipelineResult:
        ...
        # Process sources
        stats = self.process_sources(sources)
...
def process_lovdata_sources(sources: List[Dict[str, str]], bronze_dir: Path, silver_dir: Path) -> Dict[str, Any]:
    """Process Lovdata sources from Bronze to Silver."""
    from ..parsers.lovdata_parser import parse_lovdata_html
    from ..cleaners.legal_text_cleaner import clean_legal_text, normalize_text, compute_stable_hash
    ...
    for source in sources:
        ...
        try:
            ...
            sections = parse_lovdata_html(html_content, source["url"], "bronze_hash")
            ...
            for section in sections:
                ...
                # Clean text content
                if "text_plain" in section:
                    section["text_plain"] = clean_legal_text(section["text_plain"])
                    section["text_plain"] = normalize_text(section["text_plain"])
