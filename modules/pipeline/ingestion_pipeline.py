...
class IngestionPipeline(BasePipeline):
    ...
    def __init__(self, pipeline_name: str, fetcher_func: Callable[[Dict[str, str], Path], Dict[str, Any]]):
    ...
    def ingest_source(self, source: Dict[str, str]) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.get("success", True) and "error" not in result:
                ...
                self.result.add_error(f"Failed to ingest {source.get('source_id', 'unknown')}: {result.get('error', 'Unknown error')}")
            
            return result
            
        except Exception as e:
            error_msg = f"Error ingesting {source.get('source_id', 'unknown')}: {str(e)}"
            self.result.add_error(error_msg)
            return {"source_id": source.get("source_id", ""), "error": str(e), "success": False}
    ...
    def execute(self) -> PipelineResult:
        ...
        log.info(f"Ingestion complete: {successful}/{len(sources)} successful, {changed} files changed")
