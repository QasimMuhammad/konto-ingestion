...
class ValidationResult:
    ...
    def add_error(self, field: str, message: str, value: Any = None, error_type: str = "validation") -> None:
        """Add an error to the result."""
        error = ValidationError(field, message, value, error_type, "error")
        self.errors.append(error)
        self.is_valid = False
    
    def add_warning(self, field: str, message: str, value: Any = None, error_type: str = "validation") -> None:
        """Add a warning to the result."""
        warning = ValidationError(field, message, value, error_type, "warning")
        self.warnings.append(warning)
    
    def add_info(self, field: str, message: str, value: Any = None, error_type: str = "info") -> None:
    ...
    def get_warnings_by_type(self, error_type: str) -> List[ValidationError]:
        ...
        return [warning for warning in self.warnings if warning.error_type == error_type]
...
class DataValidator:
            ...
            for validator in self.validation_rules[field]:
                try:
                    if not validator(value, context):
                        result.add_error(field, f"Validation failed for {field}", value, "field_validation")
                except Exception as e:
                    result.add_error(field, f"Validation error for {field}: {str(e)}", value, "validation_exception")
    ...
    def validate_record(self, record: Dict[str, Any], schema: Dict[str, Any] = None) -> ValidationResult:
        """Validate a complete record."""
        result = ValidationResult(is_valid=True)
        
        # Validate each field
        for field, value in record.items():
            field_result = self.validate_field(field, value, {"record": record})
    ...
    def validate_dataset(self, dataset: List[Dict[str, Any]], schema: Dict[str, Any] = None) -> ValidationResult:
    ...
    def _apply_common_rules(self, field: str, value: Any, result: ValidationResult, context: Dict[str, Any]) -> None:
        """Apply common validation rules."""
        # Check for required fields
        if value is None or value == "":
            if field in ["source_url", "sha256", "domain"]:  # Common required fields
                result.add_error(field, f"Required field {field} is missing or empty", value, "required_field")
        
        # Check for valid URLs
        if field == "source_url" and value:
            if not self._is_valid_url(str(value)):
                result.add_error(field, f"Invalid URL format: {value}", value, "invalid_url")
        
        # Check for valid SHA256 hashes
        if field == "sha256" and value:
            if not self._is_valid_sha256(str(value)):
                result.add_error(field, f"Invalid SHA256 hash: {value}", value, "invalid_hash")
        
        # Check for valid domains
        if field == "domain" and value:
            valid_domains = ["tax", "accounting", "reporting"]
            if value not in valid_domains:
                result.add_error(field, f"Invalid domain: {value}. Must be one of {valid_domains}", value, "invalid_domain")
    
    def _validate_record_level(self, record: Dict[str, Any], result: ValidationResult) -> None:
        ...
        if "source_url" in record and "sha256" in record:
            if not record["source_url"] and record["sha256"]:
                result.add_warning("metadata", "SHA256 provided but no source URL", None, "inconsistent_metadata")
        ...
        if "domain" in record and "source_type" in record:
            ...
            if domain == "tax" and source_type not in ["law", "forskrift", "rates", "guidance"]:
                result.add_warning("source_type", f"Unusual source type for tax domain: {source_type}", source_type, "unusual_combination")
    
    def _validate_dataset_level(self, dataset: List[Dict[str, Any]], result: ValidationResult) -> None:
        ...
        if duplicate_hashes:
            result.add_warning("dataset", f"Found {len(duplicate_hashes)} duplicate records", duplicate_hashes, "duplicates")
        ...
        if len(unique_domains) > 1:
            result.add_info("dataset", f"Dataset contains multiple domains: {list(unique_domains)}", list(unique_domains), "multi_domain")
    ...
    def generate_report(self, result: ValidationResult, output_file: Optional[Path] = None) -> Dict[str, Any]:
