...
class SchemaValidator:
    ...
    def validate_with_schema(self, data: Union[Dict[str, Any], List[Dict[str, Any]]], schema_name: str) -> ValidationResult:
        """Validate data against a registered Pydantic schema."""
        if schema_name not in self.schemas:
            result = ValidationResult(is_valid=False)
            result.add_error("schema", f"Schema '{schema_name}' not found", schema_name, "missing_schema")
            return result
    ...
    def _validate_record_with_schema(self, record: Dict[str, Any], schema_class: Type[BaseModel], schema_name: str) -> ValidationResult:
        ...
        try:
            ...
            # Add success info
            result.add_info("validation", f"Record validated successfully with schema {schema_name}", None, "schema_validation")
            ...
            for error in e.errors():
                ...
                result.add_error(field, f"Schema validation error: {message}", None, f"pydantic_{error_type}")
            
            log.error(f"Schema validation failed for {schema_name}: {e}")
        
        except Exception as e:
            result.is_valid = False
            result.add_error("schema", f"Unexpected validation error: {str(e)}", None, "validation_exception")
            log.error(f"Unexpected error during schema validation: {e}")
        
        return result
    
    def _validate_list_with_schema(self, records: List[Dict[str, Any]], schema_class: Type[BaseModel], schema_name: str) -> ValidationResult:
        """Validate a list of records against a Pydantic schema."""
        result = ValidationResult(is_valid=True)
        
        if not records:
            result.add_warning("dataset", "Empty dataset provided for validation", None, "empty_dataset")
            return result
        
        # Validate each record
        for i, record in enumerate(records):
            record_result = self._validate_record_with_schema(record, schema_class, schema_name)
    ...
    def _validate_dataset_consistency(self, records: List[Dict[str, Any]], result: ValidationResult, schema_name: str) -> None:
        ...
        for field in unique_fields:
            if field in records[0] if records else False:
                ...
                if duplicates:
                    result.add_warning(field, f"Found {len(duplicates)} duplicate {field} values", duplicates, "duplicates")
        ...
        if len(unique_domains) > 1:
            result.add_info("domain", f"Dataset contains multiple domains: {list(unique_domains)}", list(unique_domains), "multi_domain")
        
        # Check for consistent publishers
        publishers = [record.get("publisher") for record in records if record.get("publisher")]
        unique_publishers = set(publishers)
        
        if len(unique_publishers) > 1:
            result.add_info("publisher", f"Dataset contains multiple publishers: {list(unique_publishers)}", list(unique_publishers), "multi_publisher")
    
    def validate_file(self, file_path: Path, schema_name: str) -> ValidationResult:
        """Validate a JSON file against a schema."""
        if not file_path.exists():
            result = ValidationResult(is_valid=False)
            result.add_error("file", f"File not found: {file_path}", str(file_path), "file_not_found")
            return result
        ...
        try:
            ...
            result = ValidationResult(is_valid=False)
            result.add_error("file", f"Invalid JSON in file {file_path}: {str(e)}", str(file_path), "json_decode_error")
            return result
        
        except Exception as e:
            result = ValidationResult(is_valid=False)
            result.add_error("file", f"Error reading file {file_path}: {str(e)}", str(file_path), "file_read_error")
            return result
    
    def validate_directory(self, directory: Path, schema_mapping: Dict[str, str]) -> Dict[str, ValidationResult]:
        ...
        for json_file in json_files:
            ...
            # Log result
            if result.is_valid:
                log.info(f"✓ {file_name}: Valid ({result.error_count} errors, {result.warning_count} warnings)")
            else:
                log.error(f"✗ {file_name}: Invalid ({result.error_count} errors, {result.warning_count} warnings)")
