...
def parse_mva_rates(html: str, source_url: str, sha256: str) -> List[VatRate]:
    ...
    for table in tables:
        ...
        for row in rows:
            ...
            if len(cells) >= 2:
                ...
                # Skip header rows
                if any(header in cols[0].lower() for header in ["type", "sats", "rate", "kategori", "beskrivelse"]):
                    continue
    ...
    rate_sections = main_content.find_all(["div", "section", "p"], class_=re.compile(r"rate|sats|mva"))
    
    for section in rate_sections:
        text = section.get_text(" ", strip=True)
        if any(keyword in text.lower() for keyword in ["%", "prosent", "sats", "merverdiavgift"]):
...
def extract_detailed_rate_from_row(cols: List[str], source_url: str, sha256: str) -> Optional[VatRate]:
    ...
    detailed_description = create_detailed_description(description, percentage_str, category)
...
def extract_rate_from_detailed_text(text: str, source_url: str, sha256: str) -> Optional[VatRate]:
...
def determine_detailed_category(description: str) -> tuple[str, str]:
    ...
    # Food and nutrition
    if any(word in desc_lower for word in ["næringsmidler", "mat", "food", "kjøtt", "fisk", "grønnsaker"]):
        return "food_products", "reduced"
    
    # Water and sewage services
    elif any(word in desc_lower for word in ["vann", "avløp", "water", "sewage", "drikkevann"]):
        return "water_services", "reduced"
    
    # Transport and entertainment
    elif any(word in desc_lower for word in ["persontransport", "kinobilletter", "utleie", "transport", "kino", "idrett", "fornøyelse"]):
        return "transport_entertainment", "reduced"
    
    # General/standard rate
    elif any(word in desc_lower for word in ["alminnelig", "standard", "generell", "hoved"]):
...
def create_detailed_description(description: str, percentage_str: str, category: str) -> str:
