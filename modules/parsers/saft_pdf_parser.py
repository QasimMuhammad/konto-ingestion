...
class SAFTPDFParser:
    ...
    def parse_technical_description_pdf(
        self, pdf_path: Path, source_url: str, sha256: str
    ) -> List[SpecNode]:
    ...
    def parse_masterfiles_pdf(
        self, pdf_path: Path, source_url: str, sha256: str
    ) -> List[SpecNode]:
        """Parse the masterfiles PDF."""
        text = self.extract_text_from_pdf(pdf_path)
        return self._extract_nodes_from_text(
            text, source_url, sha256, prefix="MasterFiles"
        )

    def parse_generalledger_pdf(
        self, pdf_path: Path, source_url: str, sha256: str
    ) -> List[SpecNode]:
        """Parse the general ledger entries PDF."""
        text = self.extract_text_from_pdf(pdf_path)
        return self._extract_nodes_from_text(
            text, source_url, sha256, prefix="GeneralLedgerEntries"
        )

    def _extract_nodes_from_text(
        self, text: str, source_url: str, sha256: str, prefix: str = ""
    ) -> List[SpecNode]:
        ...
        for match in xml_matches:
            ...
            node = self._create_node_from_text(
                node_path.strip(), cardinality, content.strip(), source_url, sha256
            )
            if node:
                nodes.append(node)
    ...
    def _extract_nodes_from_table(
        ...
        for i, header in enumerate(headers):
            ...
            if any(word in header_lower for word in ["xml element", "element", "name"]):
                ...
                for word in ["req", "required", "mandatory", "optional"]
            ):
                req_col = i
            elif any(
                word in header_lower
                for word in ["rep", "repetition", "cardinality", "occurs"]
            ):
                cardinality_col = i
            elif any(
                word in header_lower for word in ["description", "desc", "comment"]
            ):
                description_col = i
        ...
        for row_idx, row in enumerate(table[1:], 1):
            ...
            # Create node
            node = self._create_node_from_text(
                node_path, cardinality, description, source_url, sha256, data_type
            )
            if node:
                nodes.append(node)
                from ..logger import logger
    ...
    def _determine_data_type_from_text(self, text: str) -> str:
        ...
        if any(word in text_lower for word in ["date", "dato", "time", "tid"]):
            ...
            word in text_lower
            for word in ["amount", "beløp", "sum", "money", "currency"]
        ):
            return "decimal"
        elif any(word in text_lower for word in ["number", "tall", "count", "antall"]):
            return "integer"
        elif any(
            word in text_lower for word in ["boolean", "true", "false", "yes", "no"]
        ):
            return "boolean"
        elif any(
            word in text_lower for word in ["complex", "group", "element", "container"]
        ):
            return "complex"
        else:
            return "string"
    ...
    def _extract_technical_details(self, text: str) -> List[str]:
        ...
        length_match = re.search(
            r"(\d+)\s*(?:characters?|chars?|tegn)", text, re.IGNORECASE
        )
        if length_match:
