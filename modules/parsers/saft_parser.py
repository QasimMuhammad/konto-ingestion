...
def parse_saft_page(html: str, version: str, source_url: str, sha256: str) -> List[SpecNode]:
    """
    ...
    main_content = soup.find("main") or soup.find("div", class_=re.compile(r"content|main|article"))
    if not main_content:
        main_content = soup
    ...
    nodes.extend(extract_nodes_from_code_blocks(main_content, version, source_url, sha256))
    nodes.extend(extract_nodes_from_documentation(main_content, version, source_url, sha256))

    return nodes


def extract_nodes_from_tables(main_content, version: str, source_url: str, sha256: str) -> List[SpecNode]:
    ...
    for table in tables:
        ...
        for row in rows[1:]:
            ...
            if len(cells) >= 2:
                ...
                if any(keyword in str(row_data).lower() for keyword in ["saft", "node", "element", "field"]):
                    node = create_detailed_node_from_table_data(row_data, version, source_url, sha256)
                    if node:
                        nodes.append(node)
...
def extract_nodes_from_lists(main_content, version: str, source_url: str, sha256: str) -> List[SpecNode]:
    ...
    for list_elem in lists:
        ...
        if not any(keyword in parent_text.lower() for keyword in ["saft", "node", "element", "field", "specification"]):
            continue
        ...
        for i, item in enumerate(items):
            ...
            if len(item_text) > 10:  # Minimum content length
                node = create_detailed_node_from_list_item(item_text, version, source_url, sha256)
                if node:
                    nodes.append(node)
...
def extract_nodes_from_headings(main_content, version: str, source_url: str, sha256: str) -> List[SpecNode]:
    ...
    for heading in headings:
        ...
        if any(skip in heading_text.lower() for skip in ["navigasjon", "meny", "innhold", "overskrift", "cookie"]):
            continue
        ...
        if content_text and len(content_text) > 20:  # Minimum content length
            node = create_detailed_node_from_heading(heading_text, content_text, version, source_url, sha256)
            if node:
                nodes.append(node)
...
def extract_nodes_from_code_blocks(main_content, version: str, source_url: str, sha256: str) -> List[SpecNode]:
    ...
    for code_block in code_blocks:
        ...
        if any(pattern in code_text.lower() for pattern in ["<", "xml", "saft", "node", "element"]):
            # Extract node information from code
            node = create_detailed_node_from_code(code_text, version, source_url, sha256)
            if node:
                nodes.append(node)
...
def extract_nodes_from_documentation(main_content, version: str, source_url: str, sha256: str) -> List[SpecNode]:
    """Extract nodes from documentation sections."""
    nodes: List[SpecNode] = []
    
    # Look for documentation sections
    doc_sections = main_content.find_all(["div", "section"], class_=re.compile(r"doc|spec|content"))
    
    for section in doc_sections:
        section_text = section.get_text(" ", strip=True)
        
        if any(keyword in section_text.lower() for keyword in ["saft", "node", "element", "field", "specification"]):
            # Extract node information from documentation
            node = create_detailed_node_from_documentation(section_text, version, source_url, sha256)
            if node:
                nodes.append(node)
...
def create_detailed_node_from_table_data(row_data: Dict[str, str], version: str, source_url: str, sha256: str) -> Optional[SpecNode]:
    """Create detailed node from table data."""
    # Look for node path in various columns
    node_path = ""
    for key, value in row_data.items():
        if any(keyword in key.lower() for keyword in ["path", "node", "element", "field"]):
            node_path = value
            break
    ...
    for key, value in row_data.items():
        if any(keyword in key.lower() for keyword in ["description", "beskrivelse", "desc"]):
            description = value
        elif any(keyword in key.lower() for keyword in ["cardinality", "kardinalitet", "occurs"]):
            cardinality = value
...
def create_detailed_node_from_list_item(item_text: str, version: str, source_url: str, sha256: str) -> Optional[SpecNode]:
...
def create_detailed_node_from_heading(heading_text: str, content_text: str, version: str, source_url: str, sha256: str) -> Optional[SpecNode]:
...
def create_detailed_node_from_code(code_text: str, version: str, source_url: str, sha256: str) -> Optional[SpecNode]:
...
def create_detailed_node_from_documentation(doc_text: str, version: str, source_url: str, sha256: str) -> Optional[SpecNode]:
...
def determine_priority_from_path(node_path: str) -> str:
    ...
    if any(keyword in node_path.lower() for keyword in ["header", "hoved", "main", "root"]):
        return "high"
    elif any(keyword in node_path.lower() for keyword in ["required", "påkrevd", "mandatory"]):
        return "high"
    elif any(keyword in node_path.lower() for keyword in ["optional", "valgfri", "optional"]):
        return "low"
    else:
        return "medium"
...
def determine_complexity_from_description(description: str) -> str:
    ...
    if any(word in description.lower() for word in ["beregn", "kalkuler", "formel", "regel", "logikk"]):
        return "high"
    elif any(word in description.lower() for word in ["valider", "kontroller", "sjekk"]):
        return "medium"
    else:
        return "low"


def parse_saft_documentation(html: str, version: str, source_url: str, sha256: str) -> List[SpecNode]:
    """
