...
def parse_amelding_overview(html: str, source_url: str, sha256: str) -> List[AmeldingRule]:
    """
...
def extract_rules_from_headings(main_content, source_url: str, sha256: str) -> List[AmeldingRule]:
...
def extract_rules_from_lists(main_content, source_url: str, sha256: str) -> List[AmeldingRule]:
    ...
    for list_elem in lists:
        ...
        if not any(keyword in parent_text.lower() for keyword in ["a-melding", "rapportering", "skjema", "frist", "krav"]):
            continue
...
def extract_rules_from_tables(main_content, source_url: str, sha256: str) -> List[AmeldingRule]:
    ...
    for table in tables:
        ...
        for row in rows[1:]:
            ...
            if len(cells) >= 2:
                ...
                if any(keyword in str(row_data).lower() for keyword in ["a-melding", "rapportering", "skjema"]):
                    ...
                    rules.append(AmeldingRule(
                        ...
                        requirements=extract_detailed_requirements(description, "data_structure"),
                        examples=[],
                        source_url=source_url,
                        sha256=sha256,
                        priority="medium",
...
def extract_rules_from_forms(main_content, source_url: str, sha256: str) -> List[AmeldingRule]:
    ...
    for form in forms:
        ...
        if not any(keyword in form_text.lower() for keyword in ["a-melding", "rapportering", "skjema"]):
            continue
        ...
        for input_elem in inputs:
            ...
            if input_name:
                ...
                rules.append(AmeldingRule(
                    ...
                    requirements=[f"Felt {input_name} må fylles ut" if required else f"Felt {input_name} er valgfri"],
                    examples=[],
                    source_url=source_url,
                    sha256=sha256,
...
def extract_rules_from_links(main_content, source_url: str, sha256: str) -> List[AmeldingRule]:
    ...
    for link in links:
        ...
        if any(keyword in link_text.lower() for keyword in ["a-melding", "rapportering", "skjema", "veiledning", "regel"]):
...
def extract_form_field_rules(main_content, source_url: str, sha256: str) -> List[AmeldingRule]:
    ...
    for input_elem in inputs:
        ...
        if input_name:
            ...
            description = label.get_text(" ", strip=True) if label else f"Felt {input_name}"
            ...
            if maxlength:
                requirements.append(f"Felt {input_name} kan maksimalt ha {maxlength} tegn")
            if minlength:
                ...
                rules.append(AmeldingRule(
                    ...
                    source_url=source_url,
                    sha256=sha256,
                    technical_details=[f"Type: {input_type}", f"Navn: {input_name}"],
...
def extract_validation_rules(main_content, source_url: str, sha256: str) -> List[AmeldingRule]:
    """Extract validation rules and constraints."""
    rules: List[AmeldingRule] = []
    
    # Look for validation-related content
    validation_elements = main_content.find_all(text=re.compile(r"validering|valider|gyldig|ugyldig|feil|error"))
        ...
        if parent:
            ...
            if len(context) > 20:
                ...
                rules.append(AmeldingRule(
                    ...
                    category="form_guidance",
                    applies_to=["A-melding data"],
                    requirements=extract_detailed_requirements(context, "form_guidance"),
                    examples=[],
                    source_url=source_url,
                    sha256=sha256,
                    priority="high",
...
def extract_submission_rules(main_content, source_url: str, sha256: str) -> List[AmeldingRule]:
    """Extract submission rules and deadlines."""
    rules: List[AmeldingRule] = []
    
    # Look for deadline and submission content
    deadline_elements = main_content.find_all(text=re.compile(r"frist|deadline|innlever|submission|måned|år"))
        ...
        if parent:
            ...
            if len(context) > 20:
                ...
                rules.append(AmeldingRule(
                    ...
                    requirements=extract_detailed_requirements(context, "submission_deadlines"),
                    examples=[],
                    source_url=source_url,
                    sha256=sha256,
                    priority="high",
...
def extract_business_logic_rules(main_content, source_url: str, sha256: str) -> List[AmeldingRule]:
    """Extract business logic rules and calculations."""
    rules: List[AmeldingRule] = []
    
    # Look for calculation and business logic content
    calc_elements = main_content.find_all(text=re.compile(r"beregn|kalkuler|formel|regel|logikk"))
        ...
        if parent:
            ...
            if len(context) > 20:
                ...
                rules.append(AmeldingRule(
                    ...
                    requirements=extract_detailed_requirements(context, "business_logic"),
                    examples=[],
                    source_url=source_url,
                    sha256=sha256,
                    priority="high",
...
def determine_complexity(content_text: str, technical_details: List[str]) -> str:
    ...
    if len(technical_details) > 3 or any(word in content_text.lower() for word in ["beregn", "kalkuler", "formel"]):
        return "high"
    elif len(technical_details) > 1 or any(word in content_text.lower() for word in ["valider", "kontroller"]):
        return "medium"
    else:
        return "low"
