...
def analyze_amelding_rules(data):
    ...
    analysis["avg_requirements_per_rule"] = total_requirements / len(data) if data else 0
...
def print_analysis_summary(analyses):
    ...
    for name, analysis in analyses.items():
        ...
        if "total_sections" in analysis:
            ...
            print(f"  With Validation Rules: {analysis['nodes_with_validation_rules']:,}")
            ...
            print(f"  With Technical Details: {analysis['rules_with_technical_details']:,}")
...
def print_detailed_analysis(analyses):
    ...
    for name, analysis in analyses.items():
        ...
        for key in ["domains", "source_types", "publishers", "laws", "data_types", 
                   "cardinalities", "complexity_levels", "priority_levels", 
                   "kinds", "categories", "priorities"]:
...
def validate_schemas(data_dir: Path):
    ...
    for filename, schema_class in test_files:
        ...
        print(f"  {status} {filename}: {valid_count}/{len(data)} valid ({success_rate:.1f}%)")
...
def main():
    ...
    parser.add_argument("--data-dir", default="data/silver", help="Silver data directory")
    ...
    analyses["accounting_sections"] = analyze_law_sections(data_files["accounting_sections"])
