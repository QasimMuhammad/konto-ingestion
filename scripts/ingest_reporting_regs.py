#!/usr/bin/env python3
"""
Reporting regulations ingestion script.
Fetches and stores Norwegian reporting specifications and guidance in Bronze layer.
"""
...
def load_sources(sources_path: Path, domain: str = "reporting") -> List[Dict[str, str]]:
...
def ingest_reporting_regulation(
    source: Dict[str, str], bronze_dir: Path
) -> Dict[str, Any]:
    """Ingest a single reporting regulation source."""
...
@register_script("ingest-reporting-regs")
class IngestReportingRegsScript(BaseScript):
    """Script to ingest reporting regulations to Bronze layer."""

    def __init__(self):
        super().__init__("ingest_reporting_regs")

    def _execute(self) -> int:
        """Execute the reporting regulations ingestion."""
        ...
        for source in sources:
            result = ingest_reporting_regulation(source, bronze_dir)
